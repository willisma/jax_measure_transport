...
class TestLinear(unittest.TestCase):
    ...
    def test_linear(self):
        ...
        print(f"========== NN time: {nn_time}, NNx time: {nnx_time} ==========")
...
class TestMHA(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.nnx_rng = nnx.Rngs(params=42)
        self.nnx_mha = nnx.MHA(
            self.in_dim, self.out_dim, num_heads=self.num_heads, dtype=self.dtype, rngs=self.nnx_rng
        )
...
if __name__ == "__main__":
    ...
    print(jnp.asarray(key), jax.random.fold_in(rng2, 0))  # 

    nnx_Conv = nnx.Conv(
        4, 768, kernel_size=(3, 3), dtype=jnp.float32, rngs=rng1
    )
    nnx.display(nnx_Conv)
